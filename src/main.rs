...
mod splash {
    ...
    impl Plugin for SplashPlugin {
        fn build(&self, app: &mut App) {
            ...
            app
                ...
                .add_systems(OnEnter(GameState::Splash), (lol, splash_setup))
    ...
    fn lol(asset_server: Res<AssetServer>, mut commands: Commands) {
        // Create an entity dedicated to playing our background music
        commands.spawn(AudioBundle {
            source: asset_server.load("sounds/lol.ogg"),
            ..default()
        });
    }
    ...
    fn splash_setup(mut commands: Commands, asset_server: Res<AssetServer>) {
        let icon = asset_server.load("textures/crown.png");
        // Display the logo
        commands
            .spawn((
                NodeBundle {
        ...
        commands.insert_resource(SplashTimer(Timer::from_seconds(7.0, TimerMode::Once)));
    }
...
mod game {
                ...
                if event.state == ButtonState::Pressed {
                    if let Some(key_char) = key_code_to_char(key_code) {
                        if is_char_equal(key_char, &word_typing.current, word_typing.typed) {
                            commands.spawn(AudioBundle {
                                source: asset_server.load("sounds/snd.ogg"),
                                ..default()
                            });
                            word_typing.typed += 1;
                        }
            ...
            if !(index >= current_string.len()) {
                ...
                for (_tag, mut sprite, mut transform) in query.iter_mut() {
                    if let Some(vec) = &sprite.custom_size {
                        ...
                        if vec.y > 120.0 {
                            let bg = asset_server.load("textures/crown.png");
                        ...
                        sprite.custom_size = Some(Vec2 {
                            x: vec.x,
                            y: vec.y + 1200.0 * time.delta_seconds(),
                        });
                        transform.translation.y += 1200.0 * time.delta_seconds() / 2.0;
                    } else {
    ...
    fn game_setup(
        ...
        commands.spawn((
            SpriteBundle {
                texture: asset_server.load("textures/fam.png"),
                transform: Transform {
        ...
        commands.spawn((
            SpriteBundle {
                transform: Transform {
                    translation: Vec3::new(100.0, -265.0, 200.0),
                    ..default()
                },
                sprite: Sprite {
                    color: Color::rgb(1.0, 0.5, 0.5),
                    custom_size: Some(Vec2 { x: 50.0, y: 20.0 }),
                    ..default()
                },
                ..default()
            },
...
mod menu {
    ...
    fn main_menu_setup(mut commands: Commands, asset_server: Res<AssetServer>) {
        ...
        let bg = asset_server.load("textures/splash.png");
                ...
                parent
                    ...
                    .with_children(|parent| {
                        ...
                        parent
                            ...
                            .with_children(|parent| {
                                let icon = asset_server.load("textures/durek.png");
                                parent.spawn(ImageBundle {
